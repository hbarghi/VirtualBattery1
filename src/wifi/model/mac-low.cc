  ...
  if(newState<6)
    {
      ...
      switch(m_phyState)
        ...
        case 3:
          ...
          break;
        case 4:
          energyToDecreaseJ = duration.GetSeconds () * switchCurrent * voltage;
          //NS_LOG_HADI("sw dur: " << duration << " " << energyToDecreaseJ);
          break;
        case 5:
          energyToDecreaseJ = duration.GetSeconds () * ccaCurrent * voltage;
...
MacLow::SendDataPacket (void)
{
  ...
  NS_LOG_TB("SendDataPacket " << (int)m_currentPacket->GetUid ());
...
MacLow::SendDataAfterCts (Mac48Address source, Time duration, WifiMode txMode)
{
  ...
  m_lastTxedPacketType=2;
  NS_LOG_TB("SendDataAfterCts " << (int)m_currentPacket->GetUid ());
  m_lastTxedDataPacket = m_currentPacket->Copy ();
  ForwardDown (m_currentPacket, &m_currentHdr, dataTxVector,preamble);
  m_currentPacket = 0;
}
