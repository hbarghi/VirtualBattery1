...
BasicEnergySource::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::BasicEnergySource")
    ...
    .AddAttribute ("BasicEnergySourceGamma",
                       "Input energy amount in joules per second in basic energy source.",
                       DoubleValue (0),  // in Joules
                       MakeDoubleAccessor (&BasicEnergySource::SetGamma,
                                           &BasicEnergySource::GetGamma),
...
void
BasicEnergySource::SetGamma (double gamma)
{
  NS_LOG_FUNCTION (this << gamma);
  m_gamma=gamma;
}
double
BasicEnergySource::GetGamma () const
{
  NS_LOG_FUNCTION (this);
  return m_gamma;
}
...
BasicEnergySource::CalculateRemainingEnergy (void)
{
  ...
  m_remainingEnergyJ = m_remainingEnergyJ - energyToDecreaseJ + energyToIncreaseJ;
  if(m_remainingEnergyJ<0)
